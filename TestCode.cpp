...
int main()
{
	...
	pOut->PrintMessage("Drawing a Hexagon ==> filled,  Click one point (The centre)");
	pIn->GetPointClicked(P1.x, P1.y);
	...
	pOut->PrintMessage("Testing Input ability to read strings, Write anything");
	pOut->PrintMessage("You Entered " + pIn->GetSrting(pOut) + " , Click anywhere to continue");
	...
	do
	{
		...
		case	STOP_RECORDING:
			...
			break;
		case	PLAY_RECORD:
			pOut->PrintMessage("Action: PLay a record , Click anywhere");
			break;
		case	SAVE_GRAPH:
		...
		case	PICK_COLORED_FIGURE:
			pOut->PrintMessage("Action: Pick colored figure , Click anywhere");
			break;
		case STATUS:
